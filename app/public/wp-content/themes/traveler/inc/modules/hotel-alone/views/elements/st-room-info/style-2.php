    ...
    <div class="info">
        ...
        if ( get_post_meta( get_the_ID(), 'price_by_per_person', true ) == 'on' ) : ?>
            <div class="guest info-price">
                <p><?php esc_html_e("Adult Price",ST_TEXTDOMAIN) ?></p>
                <div class="price">
                    <?php
                    $adult_price = get_post_meta(get_the_ID(),'adult_price',true);
                    echo TravelHelper::format_money($adult_price); ?><br><span class="small"><?php esc_html_e("/person/night", ST_TEXTDOMAIN) ?></span>
                </div>
            </div>
            <div class="guest info-price">
                <p><?php esc_html_e("Child Price",ST_TEXTDOMAIN) ?></p>
                <div class="price">
                    <?php
                    $child_price = get_post_meta(get_the_ID(),'child_price',true);
                    echo TravelHelper::format_money($child_price); ?><br><span class="small"><?php esc_html_e("/person/night", ST_TEXTDOMAIN) ?></span>
                </div>
            </div>
            <?php
        else: ?>
